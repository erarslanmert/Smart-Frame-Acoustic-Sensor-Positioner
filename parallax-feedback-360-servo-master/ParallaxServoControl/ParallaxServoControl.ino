...
byte SERVO_FEEDBACK_PIN = 2;
FeedBackServo myservo = FeedBackServo(SERVO_FEEDBACK_PIN);
Servo SERVO;                        // The Parallax Servo
String inByte;
int pos;
              
        // The feedback pin (from servo)
char x;
long int v;
...
void setup() {
  attach.SERVO(SERVO_PIN);
  pinMode(SERVO_FEEDBACK_PIN, INPUT);
  myservo.setServoControl(SERVO_PIN);
  myservo.setKp(1.0);
    
  Serial.begin(9600);
  delay(20);           // Wait for serial to init
...
void loop() {
  ...
  if(Serial.available())  // if data available in serial port
    ...
    myservo.rotate(pos,4);     // move servo
